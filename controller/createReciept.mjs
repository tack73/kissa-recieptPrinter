...
export default async function createReceipt(order) {
    ...
    const page = pdfDoc.addPage([204.1,151+29*length]);
    var { width, height } = page.getSize();
    height = height - 10;
    const pngImageBytes = fs.readFileSync('./assets/reciept-header.png'); //本番環境用
    // const pngImageBytes = fs.readFileSync('./assets/reciept-header-5mogi.png');//5模擬用
    ...
    order.orderItems.forEach((v,i) => {
        ...
        page.drawText(item.name, { x: 10, y: height - 92 - 29 * i - 17});
